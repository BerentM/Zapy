...
product_tuple = namedtuple('product_tuple', ['product', 'count', 'source_id'])
user_tuple = namedtuple('user_tuple', ['username', 'email', 'password'])

sources = ['Lubawa', 'Łasin', 'Bierdronka', 'Lidl', 'Auchan']
products = [
    product_tuple('jajka', 20, 1),
    product_tuple('winko', 1, 3),
    product_tuple('wafle', 2, 4),
]

shopping_list = [
    product_tuple('piwo', 6, 3)
]
...
for prod in products:
    p = Storage(product=prod.product, count=prod.count, source_id=prod.source_id)
    db.session.add(deepcopy(p))

for prod in shopping_list:
    p = Shop(product=prod.product, count=prod.count, source_id=prod.source_id)
    db.session.add(deepcopy(p))
